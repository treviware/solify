...
import { WalletData } from 'stores/tools/walletList';
...
const bpsAmount = computed(() => token.value.account.amount);
  ...
  <q-item>
    ...
    <q-item-section>
      <q-item-label class="row justify-between">
        ...
        <div class="text-caption cursor-pointer" v-if="!metadata" @click="showBps = !showBps">
          <template v-if="showBps">{{ bpsAmount }} {{ symbol }}(BPS)</template>
          <template v-else>{{ amount }} {{ symbol }}</template>
        </div>
